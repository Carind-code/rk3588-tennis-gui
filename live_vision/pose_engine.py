...
def normalize_keypoints(keypoints: np.ndarray) -> np.ndarray:
    ...
    # RKNN Toolkit 2.3 may preserve the keypoint axes as
    # (batch, 17, 3, anchors) instead of flattening them to 51.
    if keypoints.ndim == 4 and keypoints.shape[1:3] == (NUM_KEYPOINTS, 3):
        return keypoints.reshape(keypoints.shape[0], NUM_KEYPOINTS * 3, keypoints.shape[3])
    if keypoints.ndim != 3:
...
class PoseRuntime:
    def __init__(self, model_path: Path, person_threshold: float, nms_threshold: float,
                 max_persons: int, core_mask=None) -> None:
        ...
        # A live ball detector may be active at the same time.  Reserving a
        # core mask here prevents the two RKNN runtimes from fighting for all
        # three NPU cores and keeps the preview low-latency.
    ...
    def infer(self, frame: np.ndarray) -> Tuple[List[PoseResult], float]:
        ...
        # RKNN expects an explicit batch dimension: (1, 640, 640, 3).
        outputs = self.rknn.inference(inputs=[input_rgb[None]], data_format=["nhwc"])
